...
###################### get image path list ######################
IMG_EXTENSIONS = ['.jpg', '.JPG', '.jpeg', '.JPEG', '.png', '.PNG', '.ppm', '.PPM', '.bmp', '.BMP', '.npy']
...
def read_img_fromnpy(img):
    '''read image in sio to the same output as read_img
    return: Numpy float32, HWC, BGR, [0,1]'''
    img = img.astype(np.float32) / 255.
    if img.ndim == 2:
        img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR) # added by jinliang
        # img = np.expand_dims(img, axis=2)
    # some images have 4 channels
    if img.shape[2] > 3:
        img = img[:, :, :3]
