...
class LQGTRNDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.GTs, self.Noisys, self.LQs = [], [], []
        for path in self.paths_GT:
            img_GT = util.read_img(self.GT_env, path, None)
            self.GTs.append(img_GT)

        for path in self.paths_Noisy:
            img_Noisy = util.read_img(self.Noisy_env, path, None)
            self.Noisys.append(img_Noisy)
        # if self.opt['phase'] == 'train':
        for path in self.paths_LQ:
            img_LQ = util.read_img(self.LQ_env, path, None)
            self.LQs.append(img_LQ)
