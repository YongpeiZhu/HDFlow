# base model for SRFlow, a flow-based model
import logging
...
class HCFLowRescalingModel(BaseModel):
    def __init__(self, opt, step):
        ...
        if self.is_train:
            ...
            # NLL weight
            # self.cri_pix_hr = ReconstructionLoss(losstype=train_opt['pixel_criterion_hr'])
            # self.cri_pix_lr = ReconstructionLoss(losstype=train_opt['pixel_criterion_lr'])
            ...
            if train_opt['pixel_weight_lr'] > 0:
                ...
                logger.info('Remove LR pixel loss.')
    ...
    def feed_test_data(self, data):
        ...
        # self.var_L = data.to(self.device)  # Noisy
        # self.real_H = data.to(self.device)  # Noisy

    def optimize_parameters(self, step):
        # special initialization for actnorm; don't initialize when fine-tuned with gan
        ...
        if (step % self.D_update_ratio == 0 and step > self.D_init_iters) or (not self.cri_gan):
            ...
            fake_LR, fake_z1, fake_z2 = self.netG(hr=self.noisy_H, lr=self.var_L, u=None, reverse=False)
            ...
            l_g_total = 0
            if torch.isfinite(l_g_lr):
                l_g_total += l_g_lr
            # if torch.isfinite(l_g_z):
            #     l_g_total += l_g_z
            if torch.isfinite(l_g_hr):
                l_g_total += l_g_hr

            self.log_dict['l_g_total'] = l_g_total.item()
            self.log_dict['l_g_lr'] = l_g_lr.item()
            self.log_dict['l_g_z'] = l_g_z.item()
            self.log_dict['l_g_hr'] = l_g_hr.item()
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            self.fake_L_from_H, fake_z1, fake_z2 = self.netG(hr=self.noisy_H, lr=None, u=None, reverse=False, training=False)
            self.fake_L_from_H = self.Quantization(self.fake_L_from_H)

            self.forw_L = self.fake_L_from_H[:, :3, :, :]

            y_forw = torch.cat((self.fake_L_from_H[:, :3, :, :], gaussian_scale * self.gaussian_batch(self.fake_L_from_H[:, 3:, :, :].shape)), dim=1)

            # lr+z->hr
            # for heat in self.heats:
            #     for sample in range(self.n_sample):
                    # z = self.get_z(heat, seed=1, batch_size=self.var_L.shape[0], lr_shape=self.var_L.shape)
                    # self.fake_H[(heat, sample)] = self.netG(lr=y_forw,
                    #               z=None, u=None, eps_std=heat, reverse=True, training=False)
            self.fake_H = self.netG(lr=y_forw,
                                  z=None, u=None, eps_std=1.0, reverse=True, training=False)[:, :3, :, :]


        self.netG.train()

    #     return fake_z1.mean().item()
    ...
    #         else:
    #             output, fake_z1, fake_z2= self.netG(hr=self.input)
    ...
    #             self.fake_H = self.netG(hr=y_forw)[:, :3, :, :]
    ...
    def get_z(self, heat, seed=None, batch_size=1, lr_shape=None, y_label=None):
        ...
        if util.opt_get(self.opt, ['network_G', 'flowDownsampler', 'splitOff', 'enable']):
    ...
    #     out_dict = OrderedDict()
        # out_dict['LR_ref'] = self.var_L.detach()[0].float().cpu()
    ...
    def get_current_visuals(self, need_GT=True):
        ...
        # out_dict['Denoised'] = self.fake_H[(1.0, 2)].detach()[0].float().cpu()
                # out_dict[('Denoised', heat, i)] = self.fake_H[(heat, i)].detach()[0].float().cpu()

        if need_GT:
            out_dict['GT'] = self.real_H.detach()[0].float().cpu()
            # out_dict['LR'] = self.fake_L_from_H.detach()[0].float().cpu()
